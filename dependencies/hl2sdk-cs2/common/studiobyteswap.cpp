...
// Swap a .phy file
//----------------------------------------------------------------------
int ByteswapPHY( void *pDestBase, int destBaseSize, const void *pSrcBase, const int fileSize )
{
	Assert( pCollision );
	if ( !pCollision )
		return 0;
...
// Swap a .vvd file
//----------------------------------------------------------------------
int ByteswapVVD( void *pDestBase, int destBaseSize, const void *pSrcBase, const int fileSize )
{
	Q_memset( pDestBase, 0, fileSize );
...
// Swap a .vtx file
//----------------------------------------------------------------------
int ByteswapVTX( void *pDestBase, int destBaseSize, const void *pSrcBase, const int fileSize )
{
	Q_memset( pDestBase, 0, fileSize );
...
void ByteswapFrameAnimData( studiohdr_t *&pHdrSrc, mstudioanimdesc_t *pAnimDesc, int section, byte *&pDataSrc, byte *&pDataDest )
{
	...
	{
		if (pBoneFlags[j] & STUDIO_FRAME_RAWROT )
		{
		...
		if (pBoneFlags[j] & STUDIO_FRAME_RAWPOS )
		{
			// Write the vector (3 float16)
			WriteBuffer<short>( &pDataDest, &pDataSrc, 3 );
		}
	...
	for (int iFrame = 0; iFrame < numFrames; iFrame++)
	{
		for (int j = 0; j < pHdrSrc->numbones; j++)
		{
			if (pBoneFlags[j] & STUDIO_FRAME_ANIMROT )
			{
				Quaternion q = *(Quaternion48 *)(pDataSrc);
				// printf("  %7.4f %7.4f %7.4f %7.4f\n", q.x, q.y, q.z, q.w );
				// Write the quaternion (bit fields contained in 3 unsigned shorts)
			...
			if (pBoneFlags[j] & STUDIO_FRAME_ANIMPOS )
			{
				// Write the vector (3 float16)
				WriteBuffer<short>( &pDataDest, &pDataSrc, 3 );
			}
			else if (pBoneFlags[j] & STUDIO_FRAME_FULLANIMPOS )
			{
...
// Swap an .ani file
//----------------------------------------------------------------------
int ByteswapANI( studiohdr_t* pHdr, void *pDestBase, int destBaseSize, const void *pSrcBase, const int fileSize )
{
	...
	if ( g_pCompressFunc && pHdr->numanimblocks >= 2 )
	{
...
//----------------------------------------------------------------------
int ByteswapMDL( void *pDestBase, int destBaseSize, const void *pSrcBase, const int fileSize )
{
	Q_memset( pDestBase, 0, fileSize );
...
int ByteswapStudioFile( const char *pFilename, void *pOutBase, int outBaseSize, const void *pFileBase, int fileSize, studiohdr_t *pHdr, CompressFunc_t pCompressFunc )
{
	...
	{
		retVal = ByteswapMDL( pOutBase, outBaseSize, pFileBase, fileSize );
	}
	else if ( Q_stristr( pFilename, ".vvd" ) )
	{
		retVal = ByteswapVVD( pOutBase, outBaseSize, pFileBase, fileSize );
	}
	else if ( Q_stristr( pFilename, ".vtx" ) )
	{
		retVal = ByteswapVTX( pOutBase, outBaseSize, pFileBase, fileSize );
	}
	else if ( Q_stristr( pFilename, ".phy" ) )
	{
		retVal = ByteswapPHY( pOutBase, outBaseSize, pFileBase, fileSize );
	}
	else if ( Q_stristr( pFilename, ".ani" ) )
	{
		...
		{
			retVal = ByteswapANI( pHdr, pOutBase, outBaseSize, pFileBase, fileSize );
		}
	}
...
BEGIN_BYTESWAP_DATADESC( studiohdr_t )
	...
	DEFINE_FIELD( surfacepropLookup, FIELD_INTEGER ),					// void*
